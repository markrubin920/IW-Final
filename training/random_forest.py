...
class RandomForestModel():
    def __init__(self, df: pd.DataFrame, continuous_features: list, categorical_features: list, outcome: list, player_code: int, player_name: str):
        ...
        classifier = RandomForestClassifier(class_weight='balanced', max_depth=10, max_features='sqrt', min_samples_leaf=4, min_samples_split=10, n_estimators=500)
    ...
    def evaluate(self):
        ...
        plt.title(f"{self.player_name} Random Forest Model Testing")
        plt.show()
    ...
    def get_summary(self):
        return {
            "RF accuracy": float(accuracy_score(self.y_test, self.y_predict)),
            "RF precision": float(precision_score(self.y_test, self.y_predict, average="macro", zero_division=0)),
            "RF recall": float(recall_score(self.y_test, self.y_predict, average="macro", zero_division=0)),
        }
...
if __name__ == "__main__":
    ...
    judge_svm = RandomForestModel(df, continuous_features, categorical_features, outcomes, 592450, "Aaron Judge")
