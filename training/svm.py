...
class SupportVectorModel():
    def __init__(self, df: pd.DataFrame, continuous_features: list, categorical_features: list, outcome: list, player_code: int, player_name: str):
        ...
        classifier = svm.SVC(C=10, class_weight='balanced', degree=2, gamma='auto', kernel='poly')
    ...
    def get_summary(self):
        return {
            "SVM accuracy": float(accuracy_score(self.y_test, self.y_predict)),
            "SVM precision": float(precision_score(self.y_test, self.y_predict, average="macro", zero_division=0)),
            "SVM recall": float(recall_score(self.y_test, self.y_predict, average="macro", zero_division=0)),
        }
...
if __name__ == "__main__":
    ...
    judge_svm = SupportVectorModel(df, continuous_features, categorical_features, outcomes, 592450, "Aaron Judge")
